...
-- Remove HUD
function clearGlobalHUD()
  local ui = Global.UI.getXmlTable()
  if ui == nil then return end

  for i = #ui, 1, -1 do
    local attrs = ui[i].attributes
    if attrs and (attrs.id == "kts__hud_panel" or attrs.id == "scoreBoardGUI") then
      table.remove(ui, i)
    end
  end
...
function onDestroy()
  pcall(clearGlobalHUD)
end

function onEnterContainer(container)
  pcall(clearGlobalHUD)
end
...
function onPutIntoContainer(container, player_color)
  pcall(clearGlobalHUD)
end
